...
import {
  getAgentAvatarFilenameByAgentId as getAgentAvatarFilename,
  getAgentAvatarUrlByAgentId as getAgentAvatarUrl,
} from "@/lib/agents/avatars";

type AgentAvatarStatus = "ok" | "missing";

export function useAgentAvatar({ agentId }: { agentId: string }): {
  src: string | null;
  status: AgentAvatarStatus;
} {
  const [state, setState] = useState<{ src: string | null; status: AgentAvatarStatus }>({
    src: null,
    status: "missing",
  });
  ...
  useEffect(() => {
    ...
    const onVisibility = () => {
      ...
      if (didRetryOnVisibleRef.current) return;
