...
export type SchedulingStatusTone = "scheduled" | "confirmed" | "completed" | "canceled";

export function normalizeSchedulingStatus(status: string | null | undefined): SchedulingStatusTone {
...
export function withAlpha(color: string | null | undefined, alphaHex = "33"): string {
  if (!color) return `${SCHEDULING_ACCENT_HEX}${alphaHex}`;
...
export function statusBadgeClass(status: string | null | undefined): string {
  ...
  if (tone === "canceled") return "border-red-500/30 bg-red-500/10 text-red-300";
  if (tone === "completed") return "border-emerald-500/30 bg-emerald-500/10 text-emerald-300";
  if (tone === "confirmed") {
...
export function eventCardClass(status: string | null | undefined): string {
  const tone = normalizeSchedulingStatus(status);
  if (tone === "canceled") return "border-red-400/50 bg-red-400/5 text-red-200";
  if (tone === "completed") return "border-emerald-500/40 bg-emerald-500/5 text-emerald-200";
  if (tone === "confirmed") {
