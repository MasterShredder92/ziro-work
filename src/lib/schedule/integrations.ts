 ...
 */
export async function syncEventToAttendance(
  tenantId: string,
  event: LessonEvent,
): Promise<{ ok: boolean; sessionLogId?: string; reason?: string }> {
  ...
  try {
    ...
    const log = await createSessionLog(tenantId, {
      ...
      student_rate: 0,
      teacher_rate: 0,
    } as unknown as Parameters<typeof createSessionLog>[1]);

    await logAudit("schedule.attendance.sync", {
      tenantId,
      eventId: event.id,
      sessionLogId: (log as { id: string }).id,
    });
    return { ok: true, sessionLogId: (log as { id: string }).id };
  } catch (err) {
    return {
      ok: false,
 ...
 */
export async function triggerBillingForEvent(
  tenantId: string,
  event: LessonEvent,
...
export async function scheduleEventReminder(
  ...
  event: LessonEvent,
  offsetMinutes = 24 * 60,
): Promise<{ ok: boolean; scheduledFor?: string }> {
  try {
    const remindAt = new Date(
      new Date(event.startTime).getTime() - offsetMinutes * 60_000,
    );
...
export async function notifyScheduleChange(
  tenantId: string,
  event: LessonEvent,
  change: "created" | "updated" | "cancelled" | "rescheduled",
): Promise<{ ok: boolean }> {
  try {
