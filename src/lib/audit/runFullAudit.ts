...
type ColumnSpec = { type: string; nullable?: boolean };
...
export type TableAuditResult = {
  ...
  sampleRowShapeMatches: boolean;
};

export type AuditReport = {
  tables: Record<string, TableAuditResult>;
...
const EXPECTED: Record<string, TableSpec> = {
  ...
  families: {
    columns: {
      ...COMMON,
      name: { type: "text" },
      primary_email: { type: "text", nullable: true },
      primary_phone: { type: "text", nullable: true },
    },
    primaryKey: ["id"],
    foreignKeys: [],
  ...
  schedules: {
    columns: {
      ...
      student_id: { type: "uuid", nullable: true },
  ...
  payments: {
    columns: {
      ...
      status: { type: "text" },
      paid_at: { type: "timestamp with time zone", nullable: true },
    },
    primaryKey: ["id"],
    foreignKeys: [
  ...
  lifecycle: {
    columns: {
      ...
      entity_id: { type: "uuid" },
      stage: { type: "text" },
      previous_stage: { type: "text", nullable: true },
      changed_at: { type: "timestamp with time zone" },
    },
    primaryKey: ["id"],
    foreignKeys: [],
    indexes: [
...
function emptyResult(exists: boolean): TableAuditResult {
  return {
    exists,
    missingColumns: [],
    wrongTypes: [],
...
function normalizeType(t: string | null | undefined): string {
  ...
  if (v === "timestamp") return "timestamp without time zone";
  if (v === "int" || v === "int4") return "integer";
  if (v === "int8") return "bigint";
  if (v === "varchar" || v === "character varying") return "text";
  return v;
}

async function rpcOrQuery<T>(
...
function indexMatches(have: IndexRow[], want: IndexSpec): boolean {
  ...
  return have.some((idx) => {
    ...
    if (cols.length < wantCols.length) return false;
    for (let i = 0; i < wantCols.length; i++) {
      if (cols[i] !== wantCols[i]) return false;
    }
    return true;
  });
}
...
async function auditTable(
  ...
  if (!exists) return emptyResult(false);
  ...
  const colMap = new Map<string, ColumnRow>();
  if (columns) {
    for (const c of columns) colMap.set(c.column_name, c);
  }
