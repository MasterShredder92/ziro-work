...
const systemCheck: SkillDefinition = {
  title: "System Check",
...
const kpiSnapshot: SkillDefinition = {
  title: "KPI Snapshot",
...
const usageReport: SkillDefinition = {
  title: "Usage Report",
...
const agentDiagnostics: SkillDefinition = {
  title: "Agent Diagnostics",
  description: "Produce a diagnostic report for a specific agent or all agents.",
  async handler(args: SkillHandlerInput): Promise<SkillHandlerOutput> {
    const tokens = parseTokens(args.input);
    const agentSlug = tokens.keywords.find((k) => KNOWN_AGENT_SLUGS.includes(k)) ?? null;

    const { getRecentSkillExecutions } = await import(
      "../runtime/skillTelemetry"
    );
    ...
    const terminal = filtered.filter(
      (r) => r.phase === "success" || r.phase === "failure",
    );
    ...
    for (const r of terminal) {
      if (r.source === "pack") sourceCounts.pack += 1;
      else if (r.source === "db") sourceCounts.db += 1;
    ...
    const successRate =
      terminal.length > 0 ? successes / terminal.length : 0;
