...
function shouldCountScheduledBlock(block: ScheduleBlock): boolean {
  if (isCalloutBlock(block)) return false;
  if (block.block_type === "open_time" || block.block_type === "not_bookable") return false;
  return true;
}

export async function getAdminLeads(tenantId: string): Promise<Lead[]> {
...
export async function getAdminStudents(tenantId: string): Promise<Student[]> {
...
export async function getAdminTeachers(tenantId: string): Promise<Teacher[]> {
...
export function computeAdminKpis(
  ...
  const activeStudents = students.filter((s) =>
    s.status ? ACTIVE_STUDENT_STATUSES.has(s.status) : false,
  ).length;

  const activeLeads = leads.filter((l) =>
    l.stage ? ACTIVE_LEAD_STAGES.has(l.stage) : false,
  ).length;
  ...
  const scheduledLessonsThisWeek = schedule.filter((b) => {
    ...
    if (!shouldCountScheduledBlock(b)) return false;
...
export async function getAdminKpis(tenantId: string): Promise<AdminKpis> {
...
export function computeInvoiceAging(
  invoices: SquareInvoice[],
): InvoiceAgingBucket[] {
  ...
  for (const inv of invoices) {
    const status = (inv.status ?? "").toUpperCase();
    if (!OUTSTANDING_INVOICE_STATUSES.has(status)) continue;
    const total = inv.amount_cents ?? 0;
    const paid = inv.amount_paid ?? 0;
    const balance = Math.max(0, total - paid);
    if (balance <= 0) continue;

    const dueIso = inv.due_date ?? inv.invoice_date;
    if (!dueIso) continue;
...
export function computeScheduleHeatmap(
  schedule: ScheduleBlock[],
): ScheduleHeatmapCell[] {
  const map = new Map<string, ScheduleHeatmapCell>();

  for (const block of schedule) {
    if (!block.block_date || !block.start_time) continue;
    if (!shouldCountScheduledBlock(block)) continue;
...
export function computeTeacherLoad(
  ...
  for (const b of schedule) {
    if (!shouldCountScheduledBlock(b)) continue;
    const tid = b.teacher_id;
    if (!tid) continue;
    lessonsByTeacher.set(tid, (lessonsByTeacher.get(tid) ?? 0) + 1);
    if (b.start_time && b.end_time) {
      ...
      if (Number.isFinite(minutes) && minutes > 0) {
        hoursByTeacher.set(
          tid,
          (hoursByTeacher.get(tid) ?? 0) + minutes / 60,
        );
      }
    }
  }
  ...
  return teachers
    .map((t) => {
      const first = (t["first_name"] as string | undefined) ?? "";
      const last = (t["last_name"] as string | undefined) ?? "";
      const composed = `${first} ${last}`.trim();
      const explicit = (t["full_name"] as string | undefined) ?? "";
      const email = (t["email"] as string | undefined) ?? "";
      const fullName = explicit || composed || email || t.id;
      return {
        teacherId: t.id,
        teacherName: fullName,
        studentCount: studentsByTeacher.get(t.id) ?? 0,
        lessonCount: lessonsByTeacher.get(t.id) ?? 0,
        hoursScheduled: Number(
          (hoursByTeacher.get(t.id) ?? 0).toFixed(2),
        ),
      };
    })
