...
type ApiHandler<Ctx extends RouteContext = RouteContext> = (
  req: NextRequest,
  context: Ctx,
...
export interface WithApiOptions {
  ...
  name: string;
  /** Expose internal error message in response body (never expose stack). */
 ...
 */
export function withApi<Ctx extends RouteContext = RouteContext>(
  opts: WithApiOptions,
  handler: ApiHandler<Ctx>,
) {
  return async function wrapped(req: NextRequest, context: Ctx): Promise<Response | NextResponse> {
    ...
    try {
      ...
      if (!headers.get(REQUEST_ID_HEADER)) headers.set(REQUEST_ID_HEADER, requestId);
