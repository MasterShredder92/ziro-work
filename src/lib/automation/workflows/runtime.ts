...
import {
  countRunningForTenant,
  countRunningForWorkflow,
  getRun,
  getWorkflow,
  listLogs,
  logAutomation,
...
function computeBackoffMs(attempt: number, baseMs: number): number {
  ...
  return Math.min(60_000, baseMs * Math.pow(2, clamped));
}
...
export async function enqueueRun(
  ...
  if (workflow.status !== "active") throw new Error("AUTOMATION_WORKFLOW_INACTIVE");
  ...
  if (workflow.concurrency_limit && workflow.concurrency_limit > 0) {
    const wfRunning = await countRunningForWorkflow(
      input.tenantId,
      input.workflowId,
    );
  ...
  const run = await upsertRun(input.tenantId, {
    ...
    steps: [],
    attempt: 0,
    max_attempts: input.maxAttempts ?? workflow.retry_max ?? 3,
    started_at: nowIso(),
    finished_at: null,
    duration_ms: null,
    error: null,
...
function nextActionAfter(
  ...
  if (idx >= 0 && idx + 1 < actions.length) return actions[idx + 1] ?? null;
  return null;
}
...
export async function executeRun(
  ...
  if (!startRun) throw new Error("AUTOMATION_RUN_NOT_FOUND");
  if (startRun.status === "cancelled") return startRun;
  ...
  let run = await upsertRun(tenantId, {
    ...startRun,
    status: "running",
    attempt: startRun.attempt + 1,
  });
  ...
  let current: AutomationActionDef | null = actions[0] ?? null;
  const steps: AutomationRunStep[] = [...run.steps];
  ...
  while (current) {
    ...
    steps.push(step);
    run = await upsertRun(tenantId, { ...run, steps });
    ...
    lastStep.error = result.error ?? null;

    if (!result.ok) {
      lastStep.status = "failed";
      run = await upsertRun(tenantId, {
        ...run,
        steps,
      ...
      await logAutomation(tenantId, {
        ...
        data: result.error as Record<string, unknown> | undefined,
      });

      const onError = current.onError ?? "fail";
      if (onError === "continue") {
      ...
      if (onError === "retry" && run.attempt < run.max_attempts) {
        ...
        const queued = await upsertRun(tenantId, {
          ...run,
          status: "queued",
          steps,
        });
      ...
      if (run.attempt >= run.max_attempts) {
        return finishRun(run, tenantId, "dead_letter", result.error ?? null);
      }
      return finishRun(run, tenantId, "failed", result.error ?? null);
    }
    ...
    current = nextActionAfter(actions, current, result.branchTo ?? undefined);
  }
...
async function finishRun(
  ...
  const finished = await upsertRun(tenantId, {
    ...run,
    status,
  ...
  await logAutomation(tenantId, {
    ...
    message: `Run ${status}`,
    data: error ? (error as Record<string, unknown>) : undefined,
  });
  return finished;
}
...
export async function cancelRun(
  ...
  const cancelled = await upsertRun(tenantId, {
    ...run,
    status: "cancelled",
    finished_at: nowIso(),
  });
...
export async function retryDeadLetter(
  ...
  const run = await getRun(runId, tenantId);
  if (!run) return null;
  if (run.status !== "dead_letter" && run.status !== "failed") return run;
  const workflow = await getWorkflow(run.workflow_id, tenantId);
  if (!workflow) return null;

  const requeued = await upsertRun(tenantId, {
    ...run,
    status: "queued",
    attempt: 0,
    steps: [],
