...
const EMPTY_KPIS: AttendanceKpis = {
  totalRecords: 0,
  presentCount: 0,
  absentCount: 0,
  tardyCount: 0,
 ...
 */
export function computeKpis(records: AttendanceRecord[]): AttendanceKpis {
  ...
  const punctualityRate =
    punctualityBase === 0 ? 0 : clampPct((present / punctualityBase) * 100);
...
function recordsDescByDate(
  records: AttendanceRecord[],
  sessionsById: Map<string, AttendanceSession>,
): AttendanceRecord[] {
  return [...records].sort((a, b) => {
    const da = sessionsById.get(a.session_id)?.session_date ?? "";
    const db = sessionsById.get(b.session_id)?.session_date ?? "";
...
function streaksFor(
  ...
  for (let i = 0; i < ordered.length; i++) {
    ...
    if (isPresentLike) {
      ...
      absentRun = 0;
      if (seenFirstAbsent) stillCountingAbsentFromLatest = false;
      ...
      presentRun = 0;
      if (seenFirstPresent) stillCountingPresentFromLatest = false;
    } else {
...
function computeRisk(input: {
...
}): { score: number; level: AttendanceRiskLevel; flags: AttendanceFlag[] } {
  ...
  const tardinessShare =
    kpis.totalRecords === 0 ? 0 : (kpis.tardyCount / kpis.totalRecords) * 100;
  ...
  if (score >= 50) flags.push("needs_follow_up");

  const clamped = Math.max(0, Math.min(100, Math.round(score)));
  const level: AttendanceRiskLevel =
    clamped >= 75
      ? "critical"
      : clamped >= 50
        ? "high"
...
export async function listAttendance(
  studentId: string,
  dateRange: AttendanceDateRange | null | undefined,
  tenantId?: string,
): Promise<{
  records: AttendanceRecord[];
  sessions: AttendanceSession[];
}> {
  let sessionRows: AttendanceSessionRow[] = [];
  ...
  const allRecords = await listAttendanceRecords(
    { student_id: studentId },
    tenantId,
    { limit: 2000 },
  );

  if (sessionRows.length === 0 && !dateRange?.start && !dateRange?.end) {
    const sessionIds = Array.from(new Set(allRecords.map((r) => r.session_id)));
    const lookups = await Promise.all(
      sessionIds.map((id) => getAttendanceSessionById(id, tenantId)),
    );
    sessionRows = lookups.filter((s): s is AttendanceSessionRow => !!s);
  }
  ...
  return {
    records: records as AttendanceRecordRow[] as AttendanceRecord[],
    sessions: sessionRows as AttendanceSessionRow[] as AttendanceSession[],
  };
}
...
export async function getDailySummary(
  ...
  const status: AttendanceDailySummary["status"] =
    dayRecords.length === 0 ? "none" : dayRecords[0].status;
...
export async function getSession(
  ...
  return row as AttendanceSessionRow | null;
}
...
export async function getStudentAttendanceSummary(
  ...
  const { records, sessions } = await listAttendance(
    studentId,
    dateRange ?? null,
    tenantId,
  );
  ...
  return {
    ...
    windowStart: dateRange?.start ?? null,
    windowEnd: dateRange?.end ?? null,
    kpis,
