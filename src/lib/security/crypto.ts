...
type GlobalWithKey = typeof globalThis & {
  __ziro_encryption_key?: Buffer;
  __ziro_encryption_warned?: boolean;
};
const g = globalThis as GlobalWithKey;

function resolveKey(): Buffer {
  if (g.__ziro_encryption_key) return g.__ziro_encryption_key;
  const raw = process.env.ZIRO_ENCRYPTION_KEY;
  if (!raw || raw.length === 0) {
    throw new Error(
      "ZIRO_ENCRYPTION_KEY is not set. Refuse to encrypt/decrypt without a key.",
    );
  }
  // 64 hex chars -> 32 bytes
...
export function encrypt(plaintext: string, options: EncryptOptions = {}): CipherEnvelope {
  ...
  if (options.aad) envelope.aad = options.aad;
  return envelope;
}

export function decrypt(envelope: CipherEnvelope): string {
...
export function envelopeToString(envelope: CipherEnvelope): string {
  return JSON.stringify(envelope);
}

export function envelopeFromString(raw: string): CipherEnvelope {
