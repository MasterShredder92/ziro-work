...
function computeTotals(leads: LeadRow[]): LeadDashboardData["totals"] {
  let open = 0;
  let converted = 0;
  let hot = 0;
  let warm = 0;
  ...
  for (const lead of leads) {
    if (lead.converted_student_id) converted += 1;
    else if (lead.stage !== "lost") open += 1;
...
export async function getLeadSurface(
  leadId: string,
  tenantId: string = DEFAULT_TENANT_ID,
): Promise<LeadSurfaceData | null> {
...
export async function getLeadSourceStatsSummary(
  tenantId: string = DEFAULT_TENANT_ID,
