...
import type { FileFolder, FileObject, FilePermissionContext, FileShareLink } from "./types";
import {
...
const baseCtx = (role: string): FilePermissionContext => ({
  role,
  userId: "u1",
  profileId: "u1",
  tenantId: "t1",
...
function sampleFile(over: Partial<FileObject> = {}): FileObject {
  return {
    id: "f1",
    tenantId: "t1",
    folderId: "fold1",
...
function sampleFolder(over: Partial<FileFolder> = {}): FileFolder {
  return {
    id: "fold1",
    tenantId: "t1",
    parentId: "parent1",
...
export function runPermissionsEngineSelfTest(): void {
  ...
  assert.equal(
    publicShareLinkAllowedForFileInFolder(privateNoAcl, aclFolder),
    false,
    "Private file without ACL cannot bypass restricted folder via share link",
  );
  ...
  assert.equal(
    publicShareLinkAllowedForFileInFolder(privateWithAcl, aclFolder),
    true,
    "Private file with ACL entry allowed when folder ACL is non-empty",
  );
  assert.equal(
    publicShareLinkAllowedForFileInFolder(privateNoAcl, null),
    true,
    "Root-level file not blocked by folder rule",
  );
  const tenantFile = sampleFile({ visibility: "tenant", acl: [] });
  assert.equal(
    publicShareLinkAllowedForFileInFolder(tenantFile, aclFolder),
    true,
    "Tenant-visible file may use link under ACL folder",
  );
}
