...
export async function POST(req: NextRequest) {
  ...
  if (!key || !name || !description || !system_prompt_fragment) {
    return NextResponse.json(
      { error: "key, name, description, and system_prompt_fragment are required" },
      { status: 400 }
    );
  }
...
export async function PATCH(req: NextRequest) {
  ...
  if (updates.key) updates.slug = updates.key;
  if (updates.system_prompt_fragment) updates.prompt_fragment = updates.system_prompt_fragment;
  if (updates.preferred_runtime) updates.runtime = updates.preferred_runtime;
