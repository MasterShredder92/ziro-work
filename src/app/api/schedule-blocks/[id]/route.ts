...
import type { ScheduleBlockUpdate } from "@/lib/types/entities";
...
export async function PATCH(req: NextRequest, ctx: RouteContext) {
  try {
    ...
    const u = parsed.data as Record<string, unknown>;
    const targetLocationId =
      typeof u.location_id === "string" && u.location_id.trim().length > 0
    ...
    const nextBlockType =
      typeof u.block_type === "string" && u.block_type.trim().length > 0
        ? u.block_type.trim()
        : current.block_type;
    const nextTeacherId =
      typeof u.teacher_id === "string" && u.teacher_id.trim().length > 0
        ? u.teacher_id.trim()
        : current.teacher_id;
    const nextBlockDate =
      typeof u.block_date === "string" && u.block_date.trim().length > 0
        ? u.block_date.trim()
        : current.block_date;
    const nextStartTime =
      typeof u.start_time === "string" && u.start_time.trim().length > 0
        ? u.start_time.trim()
        : current.start_time;
    const nextEndTime =
      typeof u.end_time === "string" && u.end_time.trim().length > 0
        ? u.end_time.trim()
        : current.end_time;
    const nextOriginalTeacherId =
      typeof u.original_teacher_id === "string" && u.original_teacher_id.trim().length > 0
    ...
    if (patchData.checked_in === true) {
      if (!patchData.checked_in_at) patchData.checked_in_at = new Date().toISOString();
    ...
    if (
      ...
      const nextStudentId =
        typeof patchData.student_id === "string" && patchData.student_id.trim().length > 0
    ...
    const row = await updateScheduleBlock(
      id,
      tenantId,
