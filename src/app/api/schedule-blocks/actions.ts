...
export async function getScheduleBlockAction(tenantId: string, id: string) {
  await assertTenantAccess(tenantId);
...
export async function createScheduleBlockAction(
  ...
  await assertTenantAccess(tenantId);
  const blockType = input.block_type ?? "student_session";

  const teacherAssignment = await validateTeacherLocationAssignment({
    teacherId: input.teacher_id,
    locationId: input.location_id,
  });
  if (!teacherAssignment.ok) throw new Error(teacherAssignment.error);
  ...
  const normalized: Omit<ScheduleBlockInsert, "tenant_id"> = {
    ...input,
    status:
      blockType === "call_out"
        ? "available"
        : input.checked_in
          ? "booked"
          : input.status,
    checked_in_at: input.checked_in ? input.checked_in_at ?? new Date().toISOString() : input.checked_in_at,
    teacher_tally:
      input.checked_in && typeof input.teacher_tally !== "boolean"
        ? true
        : input.teacher_tally,
    original_teacher_name:
      blockType === "sub" ? input.original_teacher_name ?? null : input.original_teacher_name,
  };
...
export async function updateScheduleBlockAction(
  ...
  const nextTeacherId = input.teacher_id ?? current.teacher_id;
  const nextLocationId = input.location_id ?? current.location_id;
  const nextBlockType = input.block_type ?? current.block_type;
  const nextBlockDate = input.block_date ?? current.block_date;
  const nextStartTime = input.start_time ?? current.start_time;
  const nextEndTime = input.end_time ?? current.end_time;
  const nextOriginalTeacherId = input.original_teacher_id ?? current.original_teacher_id;
  ...
  const patch: ScheduleBlockUpdate = { ...input };
  if (patch.checked_in === true) {
    patch.checked_in_at = patch.checked_in_at ?? new Date().toISOString();
    if (typeof patch.teacher_tally !== "boolean") patch.teacher_tally = true;
    patch.status = "booked";
...
export async function deleteScheduleBlockAction(tenantId: string, id: string) {
  await assertTenantAccess(tenantId);
