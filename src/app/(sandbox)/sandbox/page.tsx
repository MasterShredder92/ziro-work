...
export default function SandboxIndexPage() {
  return (
    <div className="space-y-[var(--z-space-6)]">
      <div>
        <h1 className="text-xl font-extrabold">UI Sandbox</h1>
        <p className="mt-2 text-sm text-[var(--z-muted)]">
          Visual QA pages for reusable UI components (not linked in the app sidebar).
        </p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-[var(--z-space-4)]">
        {demos.map((d) => (
          <Link
            key={d.href}
            href={d.href}
            className="rounded-[var(--z-radius-lg)] border border-[var(--z-border)] bg-[var(--z-surface)] px-[var(--z-space-5)] py-[var(--z-space-4)] hover:border-[var(--z-border-2)] hover:bg-[color-mix(in_oklab,var(--z-surface),white_4%)] transition-colors"
          >
            <div className="text-sm font-extrabold">{d.label}</div>
            <div className="mt-1 text-xs text-[var(--z-muted)]">{d.href}</div>
          </Link>
        ))}
      </div>
    </div>
