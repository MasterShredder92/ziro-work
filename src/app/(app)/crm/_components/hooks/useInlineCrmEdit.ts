...
import {
  updateCrmRowAction,
} from "../actions/updateCrmRowAction";
...
type UseInlineCrmEditConfig = {
  resource: CrmInlineResource;
  toPatch: (args: CommitArgs) => Record<string, unknown>;
  onOptimisticUpdate: (args: CommitArgs) => void;
  onRevert: (args: CommitArgs & { value: string }) => void;
};

export function useInlineCrmEdit({
  resource,
  toPatch,
  onOptimisticUpdate,
  onRevert,
}: UseInlineCrmEditConfig) {
  ...
  const commitEditing = useCallback(async () => {
    ...
    try {
      ...
      onRevert({ ...args, value: originalValue });
      setToast({
        id: Date.now(),
        message:
  ...
  const bindInputProps = useCallback(
    (rowId: string, columnKey: string, label: string) => ({
      value: draftValue,
      onChange: (
  ...
  const isEditingCell = useCallback(
    (rowId: string, columnKey: string) => editingKey === `${rowId}:${columnKey}`,
    [editingKey],
  );
