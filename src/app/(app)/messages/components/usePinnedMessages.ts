...
export function usePinnedMessages() {
  const [state, setState] = useState<PinnedState>({ pinned: {} });

  const pinMessage = useCallback((messageId: string) => {
    if (!messageId) return false;
    let changed = false;
    setState((prev) => {
      if (prev.pinned[messageId]) return prev;
      changed = true;
      return { pinned: { ...prev.pinned, [messageId]: true } };
    });
    return changed;
  }, []);
  ...
  const togglePin = useCallback(
    (messageId: string) => {
      if (!messageId) return { pinned: false, unpinned: false };
  ...
  const isPinned = useCallback(
    (messageId: string) => Boolean(state.pinned[messageId]),
    [state.pinned],
  );
