  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 sm:p-6 pointer-events-none">
      ...
      >
          ...
          <div className="relative flex items-start justify-between">
            <div className="flex gap-4">
              ...
              <div className="pt-0.5">
                ...
                <p className="text-[10px] text-[var(--z-muted)] leading-tight font-medium uppercase tracking-widest">
                  {rubyDef.tagline}
                </p>
              </div>
            </div>
            ...
            <button 
              onClick={onClose}
              className="h-10 w-10 rounded-2xl flex items-center justify-center text-[var(--z-muted)] hover:text-white hover:bg-white/5 transition-all active:scale-90"
            >
        ...
        <div 
          ref={scrollRef}
          className="flex-1 overflow-y-auto p-6 space-y-6 scrollbar-none bg-[#0f0f12]"
        >
          {messages.map((m, i) => (
            <div key={i} className={cn("flex flex-col", m.role === "user" ? "items-end" : "items-start")}>
              <div 
                className={cn(
                  "max-w-[85%] rounded-[24px] px-5 py-3.5 text-sm leading-relaxed",
                  m.role === "user" 
                    ? "bg-[#fb923c] text-black font-bold shadow-[0_8px_24px_rgba(251,146,60,0.25)] rounded-tr-none" 
                    : "bg-[var(--z-surface-2)] text-[var(--z-fg)] border border-[var(--z-border)] rounded-tl-none"
                )}
              >
                {m.content}
              </div>
              <span className="text-[9px] font-black text-[var(--z-muted)] uppercase tracking-[0.15em] mt-2 px-1">
                {m.role === "user" ? "Owner" : "Ruby Operator"}
              </span>
            </div>
          ))}
          {loading && (
        ...
        <div className="p-6 border-t border-[var(--z-border)] bg-[#16161a] shrink-0">
          <form onSubmit={handleSubmit} className="relative group">
            ...
            <button
              ...
              className="absolute right-2 top-2 bottom-2 px-4 rounded-[14px] bg-[#fb923c] text-black font-black text-[10px] uppercase tracking-widest hover:bg-[#ea580c] disabled:opacity-30 disabled:grayscale transition-all active:scale-95"
            >
              EXECUTE
            </button>
          </form>
          <div className="mt-4 flex items-center justify-between px-1">
            <div className="flex gap-4">
              <div className="flex items-center gap-1.5">
                <div className="h-1 w-1 rounded-full bg-[#fb923c]" />
                <span className="text-[8px] font-black text-[var(--z-muted)] uppercase tracking-tighter">OS SYNC</span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className="h-1 w-1 rounded-full bg-[#00ff88]" />
                <span className="text-[8px] font-black text-[var(--z-muted)] uppercase tracking-tighter">DIRECT DB ACCESS</span>
              </div>
            </div>
            <span className="text-[8px] font-black text-[var(--z-muted)] uppercase tracking-widest opacity-30 italic">Championship Grade</span>
          </div>
        </div>
      </div>
    </div>
