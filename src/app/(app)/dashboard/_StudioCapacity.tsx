  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
      <div style={{ position: "relative", height: 6, borderRadius: 3, background: "rgba(255,255,255,0.06)", overflow: "visible" }}>
        <div style={{ position: "absolute", left: 0, top: 0, bottom: 0, width: animated ? `${Math.min(100, pct)}%` : "0%", borderRadius: 3, background: barColor, boxShadow: `0 0 8px ${barColor}88`, transition: "width 1s cubic-bezier(0.16,1,0.3,1)" }} />
        <div style={{ position: "absolute", left: "75%", top: -3, bottom: -3, width: 1.5, background: "rgba(255,255,255,0.35)", borderRadius: 1 }} />
      </div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <span style={{ fontSize: 11, fontWeight: 800, color: barColor, fontFamily: "Space Grotesk, sans-serif" }}>{pct}%</span>
        <span style={{ fontSize: 10, color: "var(--z-muted)", fontFamily: "Space Grotesk, sans-serif" }}>target 75%</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <span style={{ fontSize: 13, fontWeight: 800, color: "var(--z-fg)", fontFamily: "Space Grotesk, sans-serif" }}>{shortName(gap.locationName)}</span>
      </div>
      <div style={{ display: "flex", alignItems: "baseline", gap: 2 }}>
        <span style={{ fontSize: 36, fontWeight: 900, lineHeight: 1, color, fontFamily: "Space Grotesk, sans-serif" }}>{gap.occupied}</span>
        <span style={{ fontSize: 18, color: "var(--z-muted)", fontFamily: "Space Grotesk, sans-serif" }}>/</span>
        <span style={{ fontSize: 18, fontWeight: 600, color: "var(--z-muted)", fontFamily: "Space Grotesk, sans-serif" }}>{gap.totalCapacity}</span>
      </div>
      <div style={{ fontSize: 10, color: "var(--z-muted)", marginTop: -6, fontFamily: "Space Grotesk, sans-serif" }}>booked / weekly capacity</div>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", borderTop: "1px solid var(--z-border)", paddingTop: 10, gap: 4 }}>
        ...
        <div style={{ textAlign: "right" }}>
          <div style={{ fontSize: 14, fontWeight: 800, color: "#bf36f8", fontFamily: "Space Grotesk, sans-serif" }}>{usd(potential75)}</div>
          <div style={{ fontSize: 9, color: "var(--z-muted)", fontFamily: "Space Grotesk, sans-serif" }}>at 75% capacity</div>
        </div>
      </div>
    </div>
  );
...
export function StudioCapacity() {
  ...
  useEffect(() => {
    fetch("/api/dashboard/revenue-gaps", { cache: "no-store" })
      .then(r => r.json()).then(setGapData).catch(() => null);
    fetch("/api/dashboard/location-revenue", { cache: "no-store" })
      .then(r => r.json()).then(setRevData).catch(() => null);
  }, []);
  ...
  const shimmerCard = (
    <div style={{ height: 220, borderRadius: 16, background: "var(--z-surface)", border: "1px solid var(--z-border)", animation: "shimmer 1.6s infinite", backgroundImage: "linear-gradient(90deg, var(--z-surface) 25%, var(--z-surface-hover, rgba(255,255,255,0.04)) 50%, var(--z-surface) 75%)", backgroundSize: "200% 100%" }} />
  );
  const cards = !gapData
    ? [1, 2, 3, 4].map(i => <div key={i}>{shimmerCard}</div>)
  ...
  return (
    <>
      <style>{`
        ...
        .sc-stats-mobile { display: none; flex-direction: column; gap: 2px; margin-bottom: 12px; }
        @media (max-width: 767px) {
          .sc-grid { grid-template-columns: repeat(2, 1fr); gap: 10px; }
          .sc-stats { display: none !important; }
          .sc-stats-mobile { display: flex !important; }
        }
      `}</style>
      <div>
        {/* section header */}
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
          ...
          {gapData && (
            <span className="sc-stats" style={{ fontSize: 11, color: "var(--z-muted)", fontFamily: "Space Grotesk, sans-serif", whiteSpace: "nowrap" }}>
        ...
        {gapData && (
          <div className="sc-stats-mobile">
            <span style={{ fontSize: 11, color: "var(--z-muted)", fontFamily: "Space Grotesk, sans-serif" }}>
              <strong style={{ color: "var(--z-fg)" }}>{totalBooked}</strong> booked · <strong style={{ color: "var(--z-fg)" }}>{totalOpen}</strong> open
            </span>
            <span style={{ fontSize: 11, color: "#ef4444", fontWeight: 700, fontFamily: "Space Grotesk, sans-serif" }}>{usd(totalGapRev)}/mo unrealized</span>
          </div>
        )}
        {/* grid — 4 cols desktop, 2 cols mobile via CSS */}
        <div className="sc-grid">{cards}</div>
      </div>
    </>
  );
}
