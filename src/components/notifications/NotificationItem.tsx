...
export function NotificationItem({ event, read, onActivate }: NotificationItemProps) {
  ...
  return (
    <button
      type="button"
      onClick={() => onActivate?.(String(event.id))}
      className={cn(
        "flex w-full items-start gap-[var(--z-space-3)] rounded-[var(--z-radius-md)] border border-[var(--z-border)] bg-[var(--z-surface)] px-[var(--z-space-4)] py-[var(--z-space-3)] text-left transition-colors",
        "hover:border-[color-mix(in_oklab,var(--z-accent),transparent_55%)] hover:bg-[color-mix(in_oklab,var(--z-surface),white_3%)]",
        read && "opacity-60",
        focusRingClassName(),
      )}
    >
