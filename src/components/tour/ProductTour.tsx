...
function sameGeom(
  a: { cx: number; cy: number; r: number } | null,
  b: { cx: number; cy: number; r: number } | null,
): boolean {
  if (a === b) return true;
  if (!a || !b) return false;
...
const STEPS: TourStepDef[] = [
  ...
  },
  {
    ...
    description:
      "Eight stages from first inquiry through win-back, so you can see where someone sits in the studio journey.",
    nextLabel: "Next",
  },
  {
    id: "student",
...
function measure(selector: string): { cx: number; cy: number; r: number } | null {
...
function tooltipStyle(rect: DOMRect | null, placement: "top" | "bottom"): React.CSSProperties {
...
export function ProductTour({ open, onClose }: ProductTourProps) {
  ...
  const [geom, setGeom] = React.useState<{ cx: number; cy: number; r: number } | null>(null);
  ...
  return createPortal(
    <>
      {geom ? (
        <TourSpotlight cx={geom.cx} cy={geom.cy} r={geom.r} onBackdropClick={onClose} />
      ) : (
        <button
          type="button"
          aria-label="Close tour"
          className="fixed inset-0 z-[70] cursor-default border-0 bg-black/80 p-0"
          onClick={onClose}
        />
      )}
      <TourStep
