...
const TABLE_ORDER: SyncTableSpec[] = require(path.join(
  __dirname,
  "schema",
...
async function loadOpenApiColumns(
  targetUrl: string,
  ...
  for (const [rawName, schema] of Object.entries(schemas)) {
    ...
    if (cols.length > 0) out.set(tableName.toLowerCase(), new Set(cols));
  }
  return out;
}

function resolveFlag(name: string, fallback = false): boolean {
...
async function syncTable(
  source: ReturnType<typeof createAdminClient>,
  target: ReturnType<typeof createAdminClient>,
  spec: SyncTableSpec,
  dryRun: boolean,
  targetColumns?: Set<string>,
): Promise<{ table: string; synced: number; skipped: boolean }> {
  let synced = 0;
  let offset = 0;
    ...
    const mappedBatch = targetColumns
      ? batch.map((row) => {
          ...
          for (const [key, value] of Object.entries(row)) {
            if (targetColumns.has(key)) next[key] = value;
          }
          return next;
        })
      : batch;
...
async function main() {
  ...
  const targetColumnMap = await loadOpenApiColumns(
    cfg.targetUrl,
    cfg.targetServiceRoleKey,
  );

  const executable: SyncTableSpec[] = [];
  ...
  for (const spec of TABLE_ORDER) {
    ...
    if (!sourceExists) {
      ...
      if (strict) throw new Error(`${spec.table}: ${reason}`);
  ...
  for (const spec of executable) {
    process.stdout.write(
      `${dryRun ? "[DRY RUN] " : ""}Syncing ${spec.table} ... `,
    );
    const result = await syncTable(
      ...
      spec,
      dryRun,
      targetColumnMap.get(spec.table.toLowerCase()),
    );
    results.push(result);
