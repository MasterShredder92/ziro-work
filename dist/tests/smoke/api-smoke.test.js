...
describe("API smoke checks", () => {
    it("POST /api/messages/send rejects invalid body safely", async () => {
        const res = await sendMessagePost(jsonRequest("http://localhost/api/messages/send", {}));
        expect(res.status).toBe(400);
    });
    it("POST /api/files/upload rejects invalid body safely", async () => {
        const res = await filesUploadPost(jsonRequest("http://localhost/api/files/upload", {}));
        expect(res.status).toBe(400);
    });
    it("POST /api/scheduling/create rejects invalid body safely", async () => {
        const res = await schedulingCreatePost(jsonRequest("http://localhost/api/scheduling/create", {}));
        expect(res.status).toBe(400);
    });
    it("POST /api/billing/subscriptions handles unauthenticated request", async () => {
        const res = await billingSubscriptionsPost(jsonRequest("http://localhost/api/billing/subscriptions", {}));
        expect([400, 401, 403, 500]).toContain(res.status);
    });
    it("POST /api/automation/run rejects invalid body safely", async () => {
        const res = await automationRunPost(jsonRequest("http://localhost/api/automation/run", {}));
        expect(res.status).toBe(400);
    });
});
