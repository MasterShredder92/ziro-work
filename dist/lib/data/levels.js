...
const g = globalThis;
function store() {
    if (!g.__ziro_levels_store)
        g.__ziro_levels_store = new Map();
    return g.__ziro_levels_store;
}
export async function listLevels(programId, tenantId, opts) {
    var _a, _b, _c;
...
export async function getLevel(levelId, tenantId) {
    var _a;
    if (!tableMissing(TABLE)) {
        try {
            const supabase = clientFor(tenantId);
    ...
    const row = (_a = store().get(levelId)) !== null && _a !== void 0 ? _a : null;
    if (!row)
        return null;
