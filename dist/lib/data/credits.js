...
const TABLE = "credits";
export async function listCredits(tenantId, filter, opts) {
    var _a, _b, _c;
...
export async function createCredit(tenantId, input) {
    const supabase = clientFor(tenantId);
    const payload = Object.assign(Object.assign({ status: "active", applied_cents: 0 }, input), { tenant_id: tenantId });
...
export async function updateCredit(id, tenantId, patch) {
    const supabase = clientFor(tenantId);
...
export async function deleteCredit(id, tenantId) {
    const supabase = clientFor(tenantId);
