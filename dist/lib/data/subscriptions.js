...
export async function listSubscriptions(tenantId, filter, opts) {
    ...
    if (filter === null || filter === void 0 ? void 0 : filter.billing_plan_id)
    ...
    if (filter === null || filter === void 0 ? void 0 : filter.due_before)
        query = query.lte("next_invoice_at", filter.due_before);
...
export async function getSubscriptionById(id, tenantId) {
    const supabase = clientFor(tenantId);
...
export async function createSubscription(tenantId, input) {
    const supabase = clientFor(tenantId);
    const payload = Object.assign(Object.assign({ status: "active", quantity: 1 }, input), { tenant_id: tenantId });
    const { data, error } = await supabase
        .from(TABLE)
        .insert(payload)
        .select("*")
...
export async function updateSubscription(id, tenantId, patch) {
    const supabase = clientFor(tenantId);
