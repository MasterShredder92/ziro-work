...
const g = globalThis;
function store() {
    if (!g.__ziro_units_store)
        g.__ziro_units_store = new Map();
    return g.__ziro_units_store;
}
export async function listUnits(levelId, tenantId, opts) {
    var _a, _b, _c;
...
export async function getUnit(unitId, tenantId) {
    var _a;
    if (!tableMissing(TABLE)) {
        try {
            const supabase = clientFor(tenantId);
    ...
    const row = (_a = store().get(unitId)) !== null && _a !== void 0 ? _a : null;
    if (!row)
        return null;
