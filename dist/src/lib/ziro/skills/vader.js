...
function resolveChannel(raw) {
    if (hasKeyword(raw, ["email"]))
        return "email";
    if (hasKeyword(raw, ["sms", "text"]))
        return "sms";
    if (hasKeyword(raw, ["call", "phone"]))
        return "call";
    return "email";
}
...
function buildMessageSkill(action, recipientKind, title, description) {
    return {
        title,
        description,
        async handler(args) {
...
const messageFamily = buildMessageSkill("message_family", "family", "Message Family", "Draft and send a message to a family account.");
const messageTeacher = buildMessageSkill("message_teacher", "teacher", "Message Teacher", "Draft and send a message to a teacher.");
const messageStudent = buildMessageSkill("message_student", "student", "Message Student", "Draft and send a message to a student.");
const inboxSummary = {
    title: "Inbox Summary",
