...
export function incrementCounter(name, labels = {}, delta = 1) {
    ...
    if (current)
        ...
        state.set(key, { labels: Object.assign({}, labels), value: delta });
}
export function setGauge(name, value, labels = {}) {
    ...
    const key = labelKey(labels);
    state.set(key, { labels: Object.assign({}, labels), value });
}
export function observeHistogram(name, value, labels = {}, buckets = DEFAULT_BUCKETS) {
    const reg = getRegistry();
...
export async function timeAsync(name, fn, labels = {}) {
    ...
    try {
        const out = await fn();
        observeHistogram(name, Date.now() - started, Object.assign(Object.assign({}, labels), { outcome: "ok" }));
        return out;
    }
    catch (err) {
        observeHistogram(name, Date.now() - started, Object.assign(Object.assign({}, labels), { outcome: "error" }));
        throw err;
    }
}
...
export function renderPrometheus() {
    ...
    for (const h of snap.histograms) {
        ...
        for (const b of h.buckets) {
            lines.push(`${h.name}_bucket${renderLabels(Object.assign(Object.assign({}, h.labels), { le: String(b.le) }))} ${b.count}`);
        }
