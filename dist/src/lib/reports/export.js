...
function formatCell(value, column) {
    ...
    if (typeof value === "number") {
        if ((column === null || column === void 0 ? void 0 : column.format) === "currency") {
            return (value / 100).toFixed(2);
        }
        if ((column === null || column === void 0 ? void 0 : column.format) === "percent") {
            return `${value}%`;
        }
        return String(value);
    }
...
export function toCsv(columns, rows) {
    const lines = [];
    const header = columns.map((c) => { var _a; return escapeCsv((_a = c.label) !== null && _a !== void 0 ? _a : c.key); }).join(",");
    lines.push(header);
 ...
 */
function toXlsxBytes(columns, rows) {
    const cells = [];
    cells.push('<?xml version="1.0" encoding="UTF-8" standalone="yes"?>');
    cells.push('<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">');
    ...
    columns.forEach((c, i) => {
        var _a;
        sheet.push(`<c r="${colLetter(i)}1" t="inlineStr"><is><t>${xmlEscape((_a = c.label) !== null && _a !== void 0 ? _a : c.key)}</t></is></c>`);
    });
    sheet.push("</row>");
    rows.forEach((row, r) => {
        sheet.push(`<row>`);
        columns.forEach((c, i) => {
            ...
            if (typeof v === "number" && Number.isFinite(v)) {
                ...
                sheet.push(`<c r="${colLetter(i)}${r + 2}" t="inlineStr"><is><t>${xmlEscape(formatted)}</t></is></c>`);
            }
        });
        sheet.push("</row>");
    });
...
// ---------------------------------------------------------------------------
function toPdfBytes(title, summary, columns, rows, branding) {
    const lines = [];
    lines.push(title);
    ...
    if (columns.length > 0) {
        lines.push("");
        const header = columns.map((c) => { var _a; return (_a = c.label) !== null && _a !== void 0 ? _a : c.key; }).join(" | ");
        lines.push(header);
    ...
    if (branding === null || branding === void 0 ? void 0 : branding.footerText) {
        lines.push("");
        lines.push(String(branding.footerText));
    }
    if (branding === null || branding === void 0 ? void 0 : branding.watermark) {
        lines.push("");
...
export function exportResult(result, format) {
    var _a;
    const name = (_a = result.name) !== null && _a !== void 0 ? _a : "report";
    ...
    switch (format) {
        ...
        case "pdf":
        default: {
            const bytes = toPdfBytes(name, result.summary, result.columns, result.rows, result.pdfExportBranding);
            return {
                filename,
...
export function exportTable(name, columns, rows, format) {
    return exportResult({
        reportId: "custom",
        reportKind: "custom",
        name,
