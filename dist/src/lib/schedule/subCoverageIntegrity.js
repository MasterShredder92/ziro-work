...
function isoDateToDayOfWeek(isoDate) {
...
export async function validateSubCoverage(input) {
    const originalTeacherId = typeof input.originalTeacherId === "string" ? input.originalTeacherId.trim() : "";
    ...
    const calloutCandidates = await listScheduleBlocks(input.tenantId, {
        teacher_id: originalTeacherId,
        location_id: input.locationId,
        date_from: input.blockDate,
        date_to: input.blockDate,
    }, { limit: 200 });
    const hasSourceCallout = calloutCandidates.some((block) => block.id !== input.excludeBlockId &&
    ...
    const conflicts = await findConflictingBlocks(input.tenantId, input.teacherId, input.blockDate, input.startTime, input.endTime, input.excludeBlockId);
    ...
    const hasAvailability = (availabilityRows !== null && availabilityRows !== void 0 ? availabilityRows : []).some((row) => toMinuteOfDay(String(row.start_time)) <= toMinuteOfDay(input.startTime) &&
