...
function markdownToHtml(input) {
    ...
    function inline(s) {
        ...
        out = out.replace(/\[([^\]]+)\]\(([^)]+)\)/g, (_m, text, href) => `<a href="${escapeAttr(href)}" target="_blank" rel="noreferrer">${text}</a>`);
        return out;
    }
...
function renderAssetReference(asset) {
    ...
    const alt = escapeAttr((_a = asset.alt_text) !== null && _a !== void 0 ? _a : asset.name);
    if (asset.kind === "image") {
...
export async function renderContentItem(item, options) {
    var _a;
    const assets = options.assets
        ? options.assets
        : await listAssets(options.tenantId, { itemId: item.id });
    const assetById = new Map(assets.map((a) => [a.id, a]));
    const missingMergeFields = [];
    const embeddedAssetIds = [];
    let working = (_a = item.body) !== null && _a !== void 0 ? _a : "";
    ...
    working = working.replace(ASSET_REF_RE, (_match, ref) => {
