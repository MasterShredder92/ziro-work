function readSkillId(action) {
    const payload = action.payload;
    if (!payload || typeof payload !== "object")
        return null;
    const raw = payload.skill;
    if (typeof raw !== "string" || raw.length === 0)
        return null;
    return raw;
}
function readDomainAction(action) {
    const payload = action.payload;
    if (!payload || typeof payload !== "object")
        return null;
    const raw = payload.domainAction;
    if (typeof raw !== "string" || raw.length === 0)
        return null;
    return raw;
}
export function resolveCrossAppAction(action) {
    if (action.intent === "nav" && action.href) {
        return { href: action.href, detail: "Opened destination" };
    }
    if (action.intent !== "custom")
        return null;
