...
function ctxMatches(entry, ctx) {
    var _a;
    if (entry.principalType === "user")
        return entry.principalId === ctx.userId;
    if (entry.principalType === "profile")
        return entry.principalId === ((_a = ctx.profileId) !== null && _a !== void 0 ? _a : ctx.userId);
    if (entry.principalType === "role")
        return entry.principalId === ctx.role;
    return false;
}
function aclGrants(entries, ctx, scope) {
    if (!entries || entries.length === 0)
        return false;
    for (const entry of entries) {
        if (!ctxMatches(entry, ctx))
            continue;
...
function isOwner(file, ctx) {
    var _a;
    if (!file.ownerId)
        return false;
    return file.ownerId === ((_a = ctx.userId) !== null && _a !== void 0 ? _a : ctx.profileId);
}
function isFolderOwner(folder, ctx) {
    var _a;
    if (!(folder === null || folder === void 0 ? void 0 : folder.ownerId))
        return false;
    return folder.ownerId === ((_a = ctx.userId) !== null && _a !== void 0 ? _a : ctx.profileId);
}
function roleAllowsWrite(role) {
    return hasPermission(role, "files.write");
}
function roleAllowsRead(role) {
    return hasPermission(role, "files.read");
}
function roleAllowsShare(role) {
    return hasPermission(role, "files.share") || role === "admin" || role === "director";
}
function roleAllowsSign(role) {
...
export function computeFilePermissions(input) {
    ...
    if (file.visibility === "public")
        canRead = true;
    if (file.visibility === "tenant" && roleAllowsRead(role))
        canRead = true;
...
export function evaluateShareLink(link, input) {
    var _a, _b;
    const now = (_a = input.now) !== null && _a !== void 0 ? _a : new Date();
    const meta = (_b = link.metadata) !== null && _b !== void 0 ? _b : {};
...
export function roleDefaultVisibility(role) {
    if (role === "admin" || role === "director")
        return "tenant";
    if (role === "teacher")
        return "tenant";
 ...
 */
export function publicShareLinkAllowedForFileInFolder(file, folder) {
    var _a, _b;
    if (!folder)
        return true;
    const folderAcl = (_a = folder.acl) !== null && _a !== void 0 ? _a : [];
    if (folderAcl.length === 0)
        return true;
    if (file.visibility === "public" || file.visibility === "tenant" || file.visibility === "shared") {
        return true;
    }
    const fileAcl = (_b = file.acl) !== null && _b !== void 0 ? _b : [];
...
export function computeFolderAccessPermissions(input) {
    ...
    if (folder.visibility === "public")
        canRead = true;
    if (folder.visibility === "tenant" && roleAllowsRead(role))
        canRead = true;
    if (folder.visibility === "shared" && roleAllowsRead(role))
        canRead = true;
    const aclRead = aclGrants(folder.acl, context, "read") ||
        (parent ? aclGrants(parent.acl, context, "read") : false);
    const aclWrite = aclGrants(folder.acl, context, "write") ||
        (parent ? aclGrants(parent.acl, context, "write") : false);
    const aclShare = aclGrants(folder.acl, context, "share") ||
        (parent ? aclGrants(parent.acl, context, "share") : false);
    canRead = canRead || aclRead;
    const canWrite = aclWrite ||
        (roleAllowsWrite(role) && role === "teacher" && canRead);
    const canShare = aclShare ||
...
function runDevPermissionSelfChecks() {
    const baseCtx = {
        role: "teacher",
        userId: "u1",
        profileId: "u1",
    ...
    const folderInherit = Object.assign(Object.assign({}, folder), { id: "f2", visibility: "private", acl: [
            {
                principalType: "user",
                principalId: "u1",
                scopes: ["read"],
    ...
    if (!ev.allowed)
        throw new Error("permissions self-check: active link should allow");
    const expired = evaluateShareLink(Object.assign(Object.assign({}, link), { expiresAt: new Date(Date.now() - 60000).toISOString() }), {});
    if (expired.allowed) {
        throw new Error("permissions self-check: expired link should deny");
    }
    const aclFolder = Object.assign(Object.assign({}, folderInherit), { id: "f_acl", acl: [{ principalType: "user", principalId: "u9", scopes: ["read"] }] });
    if (publicShareLinkAllowedForFileInFolder(Object.assign(Object.assign({}, fileInFolder), { visibility: "private", acl: [] }), aclFolder)) {
        throw new Error("permissions self-check: private file in ACL folder should block anonymous share unless file ACL/visibility opens it");
    }
    if (!publicShareLinkAllowedForFileInFolder(Object.assign(Object.assign({}, fileInFolder), { visibility: "tenant" }), aclFolder)) {
