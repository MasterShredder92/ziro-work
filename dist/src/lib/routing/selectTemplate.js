...
export async function loadTemplatesWithSkills() {
    ...
    const { data: links } = await supabase
        ...
        .in("agent_template_id", templates.map((t) => t.id))
    ...
    return templates.map((t) => (Object.assign(Object.assign({}, t), { skills: (links || [])
            .filter((l) => l.agent_template_id === t.id)
            .map((l) => skillMap.get(l.skill_id))
            .filter(Boolean) })));
}
// Score a template against a classified task
function scoreTemplate(template, classified) {
    let score = 0;
...
export function selectBestTemplate(templates, classified) {
