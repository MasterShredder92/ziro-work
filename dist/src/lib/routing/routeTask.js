...
import { composeSystemPrompt, composeSpecialistExecutionPrompt, estimateTokens, } from "./composeSystemPrompt";
import { proposeSkill } from "./proposeSkill";
import { parseStarRoutingRules, resolveStarControlAgent, } from "./starRouting";
...
async function fetchSkillsByIds(ids) {
    if (ids.length === 0)
        return [];
    const supabase = getServiceClient();
    const { data } = await supabase
...
async function tryResolveStarControlDelegation(classification, description) {
    const supabase = getServiceClient();
    ...
    const approvedAgentIds = (starCfg.approved_agent_ids || []);
    ...
    if (!resolved) {
        console.log(`[ROUTE] Star Control: no specialist for type "${classification.task_type}" (delegation=${delegationMode}).`);
        return null;
    }
    const { data: agentRow, error: oneErr } = await supabase
        ...
        .single();
    if (oneErr || !agentRow) {
        console.warn("[ROUTE] Star Control resolved id but agent row missing:", oneErr === null || oneErr === void 0 ? void 0 : oneErr.message);
        return null;
    }
    ...
    const runtime = classification.suggested_runtime;
    const route = {
        source: "star_control",
    ...
    return {
        ...
        agentMode: agentRow.mode || "persistent",
        classification,
    };
}
// Find or create an agent for this routed task
async function resolveAgent(route, composedPrompt) {
    var _a, _b, _c;
    const supabase = getServiceClient();
    ...
    const { data: newAgent, error } = await supabase
        ...
        name: ((_a = route.template) === null || _a === void 0 ? void 0 : _a.name) || "Ephemeral Agent",
        role: ((_b = route.template) === null || _b === void 0 ? void 0 : _b.description) || "Task executor",
        status: "active",
        system_prompt: composedPrompt,
        color: "#888888",
        position_x: 0,
        position_y: 0,
        template_id: ((_c = route.template) === null || _c === void 0 ? void 0 : _c.id) || null,
        mode: "ephemeral",
        current_load: 1,
        created_by: "star",
        ...
        .single();
    if (error || !newAgent) {
        console.error(`[ROUTE] Failed to create ephemeral agent: ${error === null || error === void 0 ? void 0 : error.message}`);
        // Fallback to STAR agent
        const { data: star } = await supabase.from("agents").select("id").eq("slug", "star").single();
        return { agentId: (star === null || star === void 0 ? void 0 : star.id) || "", mode: "ephemeral", agentSkillIds: [] };
    }
    return { agentId: newAgent.id, mode: "ephemeral", agentSkillIds: [] };
}
// Main routing entry point
export async function routeTask(title, description) {
    ...
    const runtime = selectRuntime(classification.suggested_runtime, best.template.supported_runtimes);
    ...
    console.log(`[ROUTE] ${title} → template="${best.template.slug}" runtime=${runtime} score=${best.score} agent=${agentId} mode=${mode}`);
    return {
        route,
        composedPrompt,
        estimatedTokens: estimateTokens(composedPrompt),
