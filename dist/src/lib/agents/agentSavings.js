 ...
 */
export const AGENT_RATE_CONFIG = {
    ziro: {
        agentId: "ziro",
        displayName: "Ziro",
...
export function taskSavedUsd(task) {
    var _a, _b;
    const rate = (_b = (_a = AGENT_RATE_CONFIG[task.agentId]) === null || _a === void 0 ? void 0 : _a.hourlyRateUsd) !== null && _b !== void 0 ? _b : 25;
    return (task.minutesSpent / 60) * rate;
}
/** Build per-agent summaries from a list of tasks */
export function buildAgentSummaries(tasks) {
    const byAgent = {};
    ...
    return Object.keys(AGENT_RATE_CONFIG).map((agentId) => {
        var _a;
        const cfg = AGENT_RATE_CONFIG[agentId];
        const agentTasks = (_a = byAgent[agentId]) !== null && _a !== void 0 ? _a : [];
        ...
        return {
            ...
            totalSavedUsd,
            ownerTasksRequired,
            tasks: agentTasks.sort((a, b) => new Date(b.completedAt).getTime() - new Date(a.completedAt).getTime()),
        };
    });
}
/** Build a period summary (week or month) */
export function buildPeriodSummary(label, startDate, endDate, allTasks) {
