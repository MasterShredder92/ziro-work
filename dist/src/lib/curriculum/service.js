...
import { getProgram, listLessons, listLevels, listMaterials, listPrograms, listUnits, } from "./queries";
async function buildLessonNodes(unitId, tenantId) {
...
async function buildUnitNodes(levelId, tenantId) {
...
async function buildLevelNodes(programId, tenantId) {
...
async function buildProgramNode(programId, tenantId) {
...
function countsForProgram(node) {
    let totalUnits = 0;
    let totalLessons = 0;
...
async function buildCurriculumTree(tenantId) {
    const programs = await listPrograms(tenantId);
    const programNodes = await Promise.all(programs.map(async (program) => {
        const levels = await buildLevelNodes(program.id, tenantId);
        return { program, levels };
    }));
    return {
...
export async function getCurriculumDashboard(tenantId) {
...
export async function getProgramSurface(programId, tenantId) {
    await assertTenantAccess(tenantId);
...
export async function getLessonSurface(lessonId, tenantId) {
    await assertTenantAccess(tenantId);
    ...
    const materials = await listMaterials(lesson.id, tenantId);
    const completions = await listStudentProgressData({ lesson_id: lesson.id }, tenantId, { limit: 100 });
    const studentsStarted = new Set(completions.filter((c) => c.status !== "not_started").map((c) => c.student_id)).size;
    const studentsCompleted = new Set(completions.filter((c) => c.status === "completed").map((c) => c.student_id)).size;
...
async function findLessonById(lessonId, tenantId) {
    const programs = await listPrograms(tenantId);
