...
export async function POST() {
    ...
    if (upsertError) {
        return Response.json({ ok: false, error: upsertError.message || "FAILED_TO_SEED_LOCATIONS" }, { status: 500 });
    }
    const normalizedLocations = Array.isArray(upsertedLocations)
        ? upsertedLocations
        : [];
    if (normalizedLocations.length > 0) {
        const firstLocationId = typeof ((_a = normalizedLocations[0]) === null || _a === void 0 ? void 0 : _a.id) === "string" ? normalizedLocations[0].id : null;
        if (firstLocationId) {
            ...
            if (!linksError && (!Array.isArray(existingLinks) || existingLinks.length === 0)) {
                await rawClient
                    .from("profile_locations")
                    .upsert([{ profile_id: profileId, location_id: firstLocationId }], { onConflict: "profile_id,location_id" });
            }
        }
    }
