 ...
 */
function computeMetrics(rows, mtdStart, mtdEnd, nextStart, nextEnd, today) {
    // Rows due this month
    const mtdRows = rows.filter((r) => r.due_date && r.due_date >= mtdStart && r.due_date <= mtdEnd);
    ...
    const collected = rows
        ...
        if (status === "PAID" && dueDate && dueDate >= mtdStart && dueDate <= mtdEnd)
            return true;
        if (paidAt && paidAt >= mtdStart && paidAt <= mtdEnd)
            return true;
        return false;
    })
    ...
    const totalInvoiced = mtdRows.reduce((s, r) => { var _a, _b; return s + ((_b = (_a = r.requested_amount) !== null && _a !== void 0 ? _a : r.amount_cents) !== null && _b !== void 0 ? _b : 0); }, 0);
    ...
    const outstanding = rows
        ...
        r.due_date <= today)
        .reduce((s, r) => { var _a; return s + ((_a = r.amount_cents) !== null && _a !== void 0 ? _a : 0); }, 0);
    const overdueCount = rows.filter((r) => (r.status === "UNPAID" || r.status === "PARTIALLY_PAID") &&
        r.due_date &&
        r.due_date < today).length;
    return {
        collected,
        totalInvoiced,
        discounted,
...
export async function GET(_req) {
    try {
        ...
        if (error)
            throw error;
        const invoices = allInvoices !== null && allInvoices !== void 0 ? allInvoices : [];
