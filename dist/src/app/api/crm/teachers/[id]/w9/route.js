...
export async function POST(req, ctx) {
    ...
    const { legal_name, business_name, tax_classification, tax_classification_other, address, city, state, zip, tin_type, tin, // raw TIN from client — we derive tin_encrypted and tin_last_four server-side
    signature_name, exempt_payee_code, fatca_exemption_code, } = body;
    // Basic validation
    ...
    const { data: w9, error: w9Error } = await db
        .from("teacher_w9")
        .upsert({
        teacher_id: id,
        tenant_id: tenantId,
    ...
    const _b = w9, { tin_encrypted: _omitTin } = _b, safeW9 = __rest(_b, ["tin_encrypted"]);
