...
export async function POST(req) {
    ...
    const { name, slug, role, purpose, instructions, system_prompt, mode = "ephemeral", owner_type = "user", color = "#00ff88", usage_triggers = [], auto_use_by_ziro = true, profile_summary, business_context = "music_school", template_id, zirorb_id, zirorb_sort = 0, } = body;
    if (!name || !role) {
    ...
    if (finalSlug === "star") {
        return NextResponse.json({ error: "Cannot create an agent with slug 'star'. Configure Star in Star Config." }, { status: 400 });
    }
...
async function isStarAgent(supabase, id) {
    const { data } = await supabase.from("agents").select("slug").eq("id", id).single();
    return (data === null || data === void 0 ? void 0 : data.slug) === "star";
}
...
export async function PATCH(req) {
    ...
    if (!action && await isStarAgent(supabase, id)) {
        return NextResponse.json({ error: "Star cannot be edited as a specialist agent. Use Star Config instead." }, { status: 403 });
    }
    ...
    if (action === "clone") {
        ...
        if (await isStarAgent(supabase, id)) {
            return NextResponse.json({ error: "Star cannot be cloned. It is a unique orchestrator." }, { status: 403 });
        }
        ...
        const { data, error } = await supabase
            ...
            zirorb_id: (_a = original.zirorb_id) !== null && _a !== void 0 ? _a : null,
            zirorb_sort: (_b = original.zirorb_sort) !== null && _b !== void 0 ? _b : 0,
            status: "active",
            is_visible_in_ui: true,
            is_archived: false,
            created_by: "admin",
        ...
        if (data) {
            ...
            if (originalSkills && originalSkills.length > 0) {
                await supabase.from("agent_skills").insert(originalSkills.map((s) => ({
                    agent_id: data.id,
                    skill_id: s.skill_id,
                    priority: s.priority,
                })));
...
export async function DELETE(req) {
    ...
    // Block deleting Star
    if (await isStarAgent(supabase, id)) {
        return NextResponse.json({ error: "Star cannot be deleted." }, { status: 403 });
    }
