...
import { activateDomain, createDomain, listBrandingDomains, markDomainVerified, removeDomain, verifyDomain, } from "@/lib/branding";
...
export async function POST(req) {
    ...
    try {
        ...
        if (!name.trim())
            return badRequest("domain_name required");
...
export async function PATCH(req) {
    ...
    try {
        ...
        if (!id)
            return badRequest("id required");
        const action = (_b = body.action) !== null && _b !== void 0 ? _b : "verify";
        let domain;
