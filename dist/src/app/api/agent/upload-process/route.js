...
export async function POST(req) {
    try {
        ...
        if (!content || !agentId) {
            return NextResponse.json({ error: "Missing content or agentId" }, { status: 400 });
        }
        ...
        if (agentId !== "sid") {
            return NextResponse.json({ error: "Only Sid can process bulk student data" }, { status: 403 });
        }
...
${allStudents === null || allStudents === void 0 ? void 0 : allStudents.map((s) => `- ${s.first_name} ${s.last_name} (ID: ${s.id})`).join("\n")}
...
If you can't parse or match a student, skip them. Only return students you can confidently match. Use null for fields with no relevant information.`;
        ...
        for (const update of updates) {
            try {
                ...
                if (update.bio)
                    updatePayload.bio = update.bio;
                if (update.goals)
                    updatePayload.goals = update.goals;
                if (update.prior_experience)
                    updatePayload.prior_experience = update.prior_experience;
                if (update.notes)
        ...
        const reply = successCount > 0
            ? `Done! ✓ I've updated ${successCount} student profile${successCount !== 1 ? "s" : ""}. ${failureCount > 0
