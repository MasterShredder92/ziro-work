...
import { createScheduleRoom, deleteScheduleRoom, updateScheduleRoom, } from "@data/scheduleRooms";
...
export async function createRoomAction(form) {
...
export async function updateRoomAction(roomId, form) {
    const ctx = await resolveScheduleContext({ requireWrite: true });
...
export async function deleteRoomAction(roomId) {
    const ctx = await resolveScheduleContext({ requireWrite: true });
    await deleteScheduleRoom(roomId, ctx.tenantId);
    revalidatePath("/schedule/rooms");
    redirect("/schedule/rooms");
}
export async function bookRoomAction(roomId, form) {
    var _a;
    const ctx = await resolveScheduleContext({ requireWrite: true });
    ...
    await assignRoomToEvent(ctx.tenantId, eventId, roomId, {
        ...
        bookedBy: (_a = ctx.session.userId) !== null && _a !== void 0 ? _a : null,
    });
    revalidatePath(`/schedule/rooms/${roomId}`);
}
