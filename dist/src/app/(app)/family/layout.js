...
import { getFamilyProfile, resolveCurrentFamilyId, } from "@/lib/portal/queries";
...
export default async function FamilyLayout({ children, }) {
    ...
    const allowedNavIds = session
        ? FAMILY_NAV_ITEMS.filter((item) => !item.scope || can(session.role, item.scope)).map((item) => item.id)
        : FAMILY_NAV_ITEMS.map((item) => item.id);
    return (_jsx(FamilyShell, { profile: profile, allowedNavIds: allowedNavIds, children: children }));
}
