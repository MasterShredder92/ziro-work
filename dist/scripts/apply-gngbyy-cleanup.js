...
const MIGRATION_FILE = path.resolve(process.cwd(), "supabase", "migrations", "20260417_cleanup_gngbyy.sql");
...
function getConfig() {
    const projectRef = process.env.SUPABASE_PROJECT_REF || "dhsyxyhtoadrqfrlmsqe";
    const serviceKey = process.env.SUPABASE_SERVICE_ROLE_KEY_GNGBYY ||
    ...
    if (!serviceKey) {
        throw new Error("Service-role key is required. Set SUPABASE_SERVICE_ROLE_KEY (or SUPABASE_SERVICE_ROLE_KEY_GNGBYY).");
    }
...
async function runViaExecSqlRpc(url, serviceKey, sql) {
...
async function verifyCleanup(projectRef, serviceKey) {
    ...
    for (const t of DROPPED_TABLES) {
        ...
        const looksMissing = msg.includes("does not exist") ||
...
async function main() {
    ...
    if (!result.ok) {
        ...
        console.error("  Tip: DDL requires the Supabase Management API. Set SUPABASE_ACCESS_TOKEN\n" +
